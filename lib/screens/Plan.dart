...
class _AnalyzePageState extends State<AnalyzePage> {
  ...
  Map<String, dynamic> extractWeeklyPlans(String content) {
    ...
    final regex =
        RegExp(r'### Week \d+([\s\S]+?)(?=### Week \d+|$)', multiLine: true);
    ...
    for (var match in matches) {
      ...
      if (weekContent != null) {
        final weekTitle =
            RegExp(r'### Week \d+').firstMatch(weekContent)?.group(0) ??
                'Unknown Week';
        final weekDetails =
            weekContent.replaceFirst(RegExp(r'### Week \d+'), '').trim();
  ...
  Map<String, dynamic> extractSuggestedRecipes(String content) {
    ...
    for (var match in matches) {
      ...
      if (recipeContent != null) {
        final recipeNameMatch =
            RegExp(r'(?<=\*\*Recipe Name\*\*:)(.*?)(?=\*\*Ingredients\*\*)')
                .firstMatch(recipeContent);
        final recipeName =
            recipeNameMatch?.group(0)?.trim() ?? 'Unnamed Recipe';
        final ingredientsMatch =
            RegExp(r'(?<=\*\*Ingredients\*\*:)(.*?)(?=\*\*Instructions\*\*)')
                .firstMatch(recipeContent);
        final ingredients = ingredientsMatch?.group(0)?.trim() ?? '';
        final instructionsMatch =
            RegExp(r'(?<=\*\*Instructions\*\*:)(.*?)(?=\*\*Recipe Name\*\*|$)')
                .firstMatch(recipeContent);
        ...
        'messages': [
          {
            'role': 'user',
            'content':
                '''I am a 35-year-old male, 6 feet tall, weighing 180 pounds, with a moderately active lifestyle. My goal is to lose 10 pounds over the next 8 weeks.
...
''',
  ...
  Widget build(BuildContext context) {
   ...
   final routeArgs =
    ...
    var Weight = routeArgs['Weight'] as String;
     var Height = routeArgs['Height'] as String;
      var FutureWeight = routeArgs['FutureWeight'] as String;
       var Gender = routeArgs['Gender'] as String;
        var Age = routeArgs['Age'] as String;
         var MedicalHistory = routeArgs['MedicalHistory'] as String;
